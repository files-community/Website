<script lang="ts">
  ...
  import { Button, ColorSwatch, HeaderChip, HyperlinkButton, PageSection } from "$lib";
...
<PageSection class="theme-{currentTheme + 1}" id="themes-section">
  ...
  <div class="themes-section-content">
    ...
    <div class="theme-chooser">
      {#each themeColors as color, i}
        <ColorSwatch on:change={replayAnimations} bind:group={currentTheme} value={i} {color}
                     aria-label="Select theme {i + 1}" />
      {/each}
    </div>
    <div class="buttons-spacer">
      <Button href="https://www.microsoft.com/store/productId/9N20KQ61LSFQ" rel="noreferrer noopener" target="_blank"
              variant="accent">
        Get Themes
      </Button>
      <HyperlinkButton
  ...
  <div bind:this={showcase} class="component-showcase" class:no-initial-delay={noInitialDelay} class:visible>
