<script lang="ts">
	...
	$: searchResults = docsPages.filter(page => page.name.toLowerCase().replace(/ /gi, "").includes((searchQuery ?? "").toLowerCase().replace(/ /gi, "")))
...
<svelte:head>
	...
	<meta content="Files - {pageTitle ? `Docs - ${pageTitle}` : 'Docs'}" property="og:title">
